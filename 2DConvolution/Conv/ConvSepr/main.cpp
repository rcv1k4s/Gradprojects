...
int main(int argc, char **argv) {
 ...
 unsigned char *grayscale; // *d_greenBlurred, *d_blueBlurred;
...
String imageName( "vikas.jpg" ); // by default
    if( argc > 2)
    {
        imageName = argv[2];
    }
...
cv::cvtColor(image, imageInputRGBA, CV_BGR2GRAY);

cv::imwrite("v1.jpg", imageInputRGBA);

 //allocate memory for the output
  imageOutputRGBA.create(image.rows, image.cols, CV_8UC1);

cv::imwrite("v2.jpg", imageOutputRGBA);
...
//printf("sizeOf d_inputImageRGBA= %d ,d_outputImageRGBA=%d, %d\n",sizeof(d_inputImageRGBA),sizeof(d_outputImageRGBA),numPixels * sizeof(int));
  ...
  checkCudaErrors(cudaMemcpy(d_inputImageRGBA, h_inputImageRGBA, sizeof(int) * numPixels, cudaMemcpyHostToDevice));
	...
	checkCudaErrors(cudaMalloc((void**)&grayscale,    sizeof(unsigned char) * numPixels));
	...
	checkCudaErrors(cudaMemset(grayscale,   0, sizeof(unsigned char) * numPixels));
...
allocateMemoryAndCopyToGPU(numRows(), numCols(), h_filter, filterWidth);
  ...
  your_gaussian_blur(h_inputImageRGBA, d_inputImageRGBA, d_outputImageRGBA, numRows(), numCols(),
				grayscale, filterWidth);
printf("vikas it is here 1 ");
  timer.Stop();
 cudaDeviceSynchronize(); checkCudaErrors(cudaGetLastError());
int err= printf("Yofffur code ran in: %f msecs.\n", timer.Elapsed());


  if (err < 0) {
    //Couldn't print! Probably the student closed stdout - bad news
    std::cerr << "Couldn't print timing information! STDOUT Closed!" << std::endl;
printf("vikas it is here 4 ");
    exit(1);
  }

printf("vikas it is here 3");
...
cv::Mat output(numRows(), numCols(), CV_8UC4, h_outputImageRGBA);
  ...
  //cv::cvtColor(output, imageOutputBGR,0);
  //output the image
cv::imwrite("ghg.jpg", output);
